...
def para2rate(
    para_all=np.array([0.025, 2.25, 0.35, 0.75, 0.5, 500, 4]), seed0=11, f_Y=IDD, ext=""
):
    ...
    for ii in range(n_trial):
        ...
        print(
            "No.{}/{}: rateE: {:.2f} Hz, rateI: {:.2f} Hz".format(
                ii + 1, n_trial, rE, rI
            )
        )
        Record_rate.append([rE, rI])
        ST_all.append([np.asarray(s_mon.t), np.asarray(s_mon.i), s_mon.spike_trains()])

    spectrum_all = PSD_analysis(ST_all)
    SSI_all = SSI_analysis(ST_all)
    Y_all = np.concatenate((Record_rate, spectrum_all, SSI_all), axis=1)
    l_output = np.shape(Y_all)[1]

    if n_trial >= 10:
        np.savez(
            "Record_Neu{}_input{}d_{}output{}d_trial{}_".format(
                N_E + N_I, l_para, f_Y.__name__, l_output, n_trial
            )
            + ext
            + "_"
    ...
    if n_trial == 1:
        showST(ST_all[0], N_E=N_E, lim=(1, 2))

    return f_Y(Y_all), ST_all
