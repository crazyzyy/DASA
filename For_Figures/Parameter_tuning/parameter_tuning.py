...
tgt_cand = [[1,5],[5,10],[5,20],[5,30],[10,20],[10,40],[10,60],[15,45],[15,60],[20,20],[20,60],[20,90],[25,50],[25,100],[25,150],[30,45],[30,100],[30,140]]
# tgt_cand = [[25,50],[25,100],[25,150]]
#tgt_cand = [[25,100]]
...
#            print(i)
    ...
    fix_model(model)    
    W0 = tf.Variable(para_cand/rs,dtype = tf.float32,trainable = True,name = 'fdasfdsa',constraint=lambda x: tf.clip_by_value(x, minv/rs, maxv/rs)) 
...
#        print(lossi)
    ...
    np.save('APT_results_{}_tgt_{}_{}.npy'.format(np.sum(id_good),tgt[0],tgt[1]),{'para_cand':para_cand,'para_good':para_good,'rate_exp':rate_exp,'rate_pred':rate_pred})
    
    #model = load_model(name+'.h5',
    #                   custom_objects={'weighted_mse': weighted_mse,"GlorotUniform": tf.keras.initializers.glorot_uniform})
    #model.predict(para_post[0:10])

    # tgt_cand = [[1,5],[5,10],[5,20],[5,30],[8,16],[10,25],[10,60],[15,45],[20,10],[20,20],[20,60],[30,45],[30,100],[25,50],[25,100],[25,150]]
    # for x in tgt_cand:
    #     print(np.sum(np.sum(np.abs(dataset['train'][1]-np.array([x])),axis = 1)<0.2))
